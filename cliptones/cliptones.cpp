// cliptones.cpp : Defines the entry point for the console application.
// Clips any out-of-range frequencies to the min or max for a particular chip.
...
#define RENAME ".cliptone.old"

int main(int argc, char *argv[])
{
	printf("VGMComp2 Tone Clipping Tool - v20200924\n\n");

    if (argc < 2) {
        printf("cliptones (-SN|-AY|-SID) <channel input>\n");
        printf("Based on the specified output chip, clips any out-of-range\n");
    ...
    for (int idx = 0; idx<row; ++idx) {
        // 0,0 means an uninitialized channel, no need to clip it
        if ((VGMDAT[0][idx] == 0) && (VGMVOL[0][idx] == 0)) continue;

        if (VGMDAT[0][idx] < minNote) {
            VGMDAT[0][idx] = minNote;
            ++clipped;
        } else if (VGMDAT[0][idx] > maxNote) {
            VGMDAT[0][idx] = maxNote;
            ++clipped;
        }
    }
