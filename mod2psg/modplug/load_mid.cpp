...
static void mid_notes_to_percussion(MIDTRACK *tp, ULONG adjust, ULONG tmin)
{
	...
	if( ton > toff ) {
		char info[32];
		sprintf_s(info,"%ld > %ld note %d", (long)ton, (long)toff, n);
...
static void mid_prog_to_notes(MIDTRACK *tp, ULONG adjust, ULONG tmin)
{
	...
	if( ton > toff ) {
		char info[40];
		sprintf_s(info,"channel %d, %ld > %ld note %d", tp->chan + 1, (long)ton, (long)toff, n);
