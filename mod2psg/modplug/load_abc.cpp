...
static void setenv(const char *name, const char *value, int overwrite)
{
	...
	sprintf_s(str, len, "%s=%s", name, value);
	putenv(str);
	free(str);
}
#endif
...
static void abc_song_to_parts(ABCHANDLE *h, char **abcparts, BYTE partp[27][2])
{
	...
	strcpy_s(buf,"A");	// initialize our temporary array
	i = 1;
	loop = 1;
	partno = 0;
	partsegno = 0;
...
static void abc_preprocess(ABCHANDLE *h, ABCMACRO *m)
{
	...
	if( m->n ) {
		k = m->n - m->name;
		for( i=0; i<14; i++ ) {
            int sizet = strlen(m->name)+1;
			char *t = new char[sizet];
			strcpy_s(t, sizet, m->name);
...
static ABCHANDLE *ABC_Init(void)
{
	...
	if( p ) {
		...
		if( *p == '-' ) {
			...
			sprintf_s(buf,"-%ld",retval->pickrandom+2);
			setenv(ABC_ENV_NORANDOMPICK, buf, 1);
		}
	}
	else {
#if 1
        retval->pickrandom = 0; // play sequentially by default, not random (tursi)
#else
		...
		sprintf_s(buf,"-%ld",retval->pickrandom); // xmms preloads the file
		setenv(ABC_ENV_NORANDOMPICK, buf, 1);
#endif
	}
	return retval;
...
static int ABC_Key(const char *p)
{
	...
	if( !strcmp(c,"Hp") || !strcmp(c,"HP") ) // highland pipes
		strcpy_s(c,"Bm");	// two sharps at c and f
...
// adds chord name and note set to list of known chords
{
	...
	if(chordsnamed > MAXCHORDNAMES - 1)
		...
		strcpy_s(chordname[chordsnamed], s);
			...
			switch(abcstate) {
				...
				case INBETWEEN:
					if( !strncmp(p,"X:",2) ) {
						...
						abc_MIDI_beat(h, ""); // reset beat array
						abc_MIDI_beatstring(h, ""); // reset beatstring
						strcpy_s(h->gchord, ""); // reset gchord string
				...
				case INHEAD:
					if( !strncmp(p,"L:",2) ) {
						sscanf_s(p+2," %d / %d", &snotelen, &snotediv);
						break;
					}
					if( !strncmp(p,"M:",2) ) {
					...
					if( !strncmp(p,"T:",2) ) {
						...
						strcpy_s(buf,m_szNames[0]);
						if( strlen(buf) ) strcat_s(buf," "); // add a space
						// don't go past 200 bytes.
						strncat_s(buf, p+t, 200-strlen(buf)-1);
					...
					if( !strncmp(p,"K:",2) ) {
						abcstate = INBODY;
						abckey = ABC_Key(p+2);
						sprintf_s(barsig, "%s%s", sig[abckey], sig[abckey]);	// reset the key signature
						p = abc_skip_word(p+2);
