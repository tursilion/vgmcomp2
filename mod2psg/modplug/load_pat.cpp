...
void pat_init_patnames(void)
{
	...
	MMSTREAM *mmcfg;
	strncpy_s(pathforpat, PATHFORPAT, PATH_MAX);
	strncpy_s(timiditycfg, TIMIDITYCFG, PATH_MAX);
	p = getenv(PAT_ENV_PATH2CFG);
	if( p ) {
		strncpy_s(timiditycfg, p, PATH_MAX - 14);
		strncpy_s(pathforpat, p, PATH_MAX - 13);
		strcat_s(timiditycfg, "/timidity.cfg");
		strcat_s(pathforpat, "/instruments");
	}
	strncpy_s(cfgsources[0], timiditycfg, PATH_MAX - 1);
	nsources = 1;
...
static void pat_read_patname(PATHANDLE *h, MMFILE *mmpat) {
	...
	strncpy_s(h->patname, ih.instrument_name, 16);
	h->patname[15] = '\0';
}
...
static void pat_get_layerheader(MMFILE *mmpat, LayerHeader *hl)
{
	...
	strncpy_s(hl->reserved, ih.instrument_name, 40);
}
...
static void pat_read_waveheader(MMSTREAM *mmpat, WaveHeader *hw, int layer)
{
	...
	_mm_read_UBYTES((BYTE *)hw, sizeof(WaveHeader), mmpat);
	strncpy_s(hw->reserved, hl.reserved, 32);
	hw->reserved[31] = 0;
...
//  envelope rates and offsets pinched from timidity's acpiano.pat sample no 1
		...
		strncpy_s(hw.reserved, midipat[gm-1], sizeof(hw.reserved));
		hw.reserved[sizeof(hw.reserved) - 1] = '\0';
		pat_setpat_inst(&hw, d, smp);
	}
	if( hw.reserved[0] )
		strncpy_s(s, hw.reserved, 32);
	else
		strncpy_s(s, midipat[gm-1], 32);
	s[31] = '\0';
	memset(d->name, 0, 32);
	strcpy_s(d->name, s);
	strncpy_s(s, midipat[gm-1], 12);
	s[11] = '\0';
	memset(d->filename, 0, 12);
	strcpy_s(d->filename, s);
}
...
static void PATsample(CSoundFile *cs, MODINSTRUMENT *q, int smp, int gm)
{
	WaveHeader hw;
	char s[256];
	sprintf_s(s, "%d:%s", smp-1, midipat[gm-1]);
	s[31] = '\0';
	memset(cs->m_szNames[smp], 0, 32);
	strncpy_s(cs->m_szNames[smp], s, 32-1);
	q->nGlobalVol = 64;
	q->nPan       = 128;
	q->uFlags     = CHN_16BIT;
	if( pat_readpat_attr(gm-1, &hw, 0) ) {
...
BOOL PAT_Load_Instruments(void *c)
{
	...
	strncpy_s(of->Headers[0]->name, "Timidity GM patches", 32);
	t = of->m_nSamples - 1;
...
BOOL CSoundFile::ReadPAT(const BYTE *lpStream, DWORD dwMemLength)
{
	...
	if( strlen(h->patname) )
		sprintf_s(buf,"%s canon %d-v (Fr. Jacques)", h->patname, h->samples);
	else
		sprintf_s(buf,"%d-voice canon (Fr. Jacques)", h->samples);
	if( strlen(buf) > 31 ) buf[31] = '\0'; // chop it of
	strcpy_s(m_szNames[0], buf);
	...
	for(t=1; t<(int)m_nSamples; t++) { // xmms modplug doesn't use slot zero
		...
		memset(s,0,32);
		if( hw.wave_name[0] )
			sprintf_s(s, "%d:%s", t, hw.wave_name);
		else {
			if( h->patname[0] )
				sprintf_s(s, "%d:%s", t, h->patname);
			else
				sprintf_s(s, "%d:Untitled GM patch", t);
		}
		s[31] = '\0';
		memset(m_szNames[t], 0, 32);
		strcpy_s(m_szNames[t], s);
	...
	if( h->patname[0] )
		strncpy_s(Headers[0]->name, h->patname, 32);
	else
		strncpy_s(Headers[0]->name, "Timidity GM patch", 32);
	t = m_nSamples - 1;
